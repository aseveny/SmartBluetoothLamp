...
public class BluetoothChatService {
	...
	private void connectionLost() {
		...
		Message msg = mHandler.obtainMessage(Bluetooth.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(Bluetooth.TOAST, "Connect device is lost");
		msg.setData(bundle);
		mHandler.sendMessage(msg);
		BluetoothChatService.this.start();
	}
	...
	private class AcceptThread extends Thread {
		...
		public void run() {
			...
			while (mState != STATE_CONNECTED) {
				...
				// If a connection was accepted // 连接已经建立成功
				if (socket != null) {
		...
		public void cancel() {// 关闭服务端的socket
			if (D)
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			// By the remote device, and the only UUID create a used to connect to socket
			try {
